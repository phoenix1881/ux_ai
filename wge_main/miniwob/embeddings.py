...
#         self._dom_neighbor_projection = Linear(int(base_embed_dim), int(projection_dim))
...
#         #dom_embeds = dom_embeds_flat.view(batch_size, num_elements, int(embed_dim / 2))
#         embed_dim = dom_embeds_flat.size(0) // (batch_size * num_dom_elems)
#         dom_embeds = dom_embeds_flat.view(batch_size, num_dom_elems, embed_dim)
...
class DOMContextEmbedder(Embedder):
    ...
    def __init__(self, input_dim, output_dim):
        ...
        self._linear = Linear(int(input_dim), int(output_dim), bias=True)
...
class HigherOrderDOMElementEmbedder(Embedder):
    def __init__(self, base_embed_dim):
        ...
        self._base_embed_dim = base_embed_dim
        self._lca_cache = {}
        self._lca_depth_start = 3
        self._lca_depth_end = 7
        
        lca_range = self._lca_depth_end - self._lca_depth_start
        
        # Calculate projection dimension while ensuring it's an integer
        projection_dim = int(base_embed_dim / lca_range)
        self._dom_neighbor_projection = Linear(base_embed_dim, projection_dim)
        self._dom_neighbors_embed_dim = projection_dim * lca_range
    ...
    def _validate_shapes(self, batch_size, num_dom_elems, base_dom_embeds):
        """Validate input shapes to catch errors early"""
        expected_flat_size = batch_size * num_dom_elems
        actual_flat_size = base_dom_embeds.size(0)
        if expected_flat_size != actual_flat_size:
            raise ValueError(
                f"Shape mismatch in base_dom_embeds:\n"
                f"Expected flat size: {expected_flat_size}\n"
                f"Actual flat size: {actual_flat_size}\n"
                f"Check batch_size ({batch_size}) and num_dom_elems ({num_dom_elems})"
            )
    ...
    def forward(self, dom_elems, base_dom_embeds):
        ...
        Args:
            dom_elems (list[list[DOMElement]]): a batch of DOMElement
                sequences to embed. All sequences must be padded.
            base_dom_embeds (Variable[FloatTensor]):
                batch_size, num_dom_elems, base_dom_embed_dim

        Returns:
            dom_embeds (Variable[FloatTensor]): shape (batch_size,
                num_dom_elems, embed_dim)
        """
        batch_size, num_dom_elems, base_embed_dim = base_dom_embeds.size()
        
        # Validate input shapes
        if base_embed_dim != self._base_embed_dim:
            raise ValueError(f"Expected base_embed_dim {self._base_embed_dim}, got {base_embed_dim}")
            
        # Flatten for processing
        try:
            base_dom_embeds_flat = base_dom_embeds.view(batch_size * num_dom_elems, base_embed_dim)
        except RuntimeError as e:
            raise RuntimeError(f"Failed to reshape base_dom_embeds: {e}\n"
                             f"Original shape: {base_dom_embeds.size()}\n"
                             f"Attempted shape: ({batch_size * num_dom_elems}, {base_embed_dim})")

        # Validate flattened dom_elems
        dom_elems_flat = flatten(dom_elems)
        if len(dom_elems_flat) != batch_size * num_dom_elems:
            raise ValueError(
                f"DOM elements count mismatch:\n"
                f"Expected: {batch_size * num_dom_elems}\n"
                f"Got: {len(dom_elems_flat)}"
            )

        # Process DOM neighbors
        dom_neighbor_embeds = []
        for k in range(self._lca_depth_start, self._lca_depth_end):
            is_neighbor_fn = lambda elem1, elem2: (
                N.is_depth_k_lca_neighbor(elem1, elem2, k, self._lca_cache) and 
                N.is_text_neighbor(elem1, elem2)
            )
            
            dom_neighbor_indices = self._get_neighbor_indices(dom_elems, is_neighbor_fn)
            
            # Compute neighbor embeddings
            neighbor_embedding = SequenceBatch.reduce_sum(
                SequenceBatch.embed(dom_neighbor_indices, base_dom_embeds_flat)
            )
            
            # Project embeddings
            projected_neighbor_embedding = self._dom_neighbor_projection(neighbor_embedding)
            dom_neighbor_embeds.append(projected_neighbor_embedding)
        ...
        try:
            ...
            sizes = [emb.size() for emb in dom_neighbor_embeds]
            raise RuntimeError(f"Failed to concatenate neighbor embeddings: {e}\n"
                             f"Individual sizes: {sizes}")

        # Process pixel neighbors
        pixel_neighbor_indices = self._get_neighbor_indices(
            dom_elems,
            lambda elem1, elem2: (
                N.is_pixel_neighbor(elem1, elem2) and 
                N.is_text_neighbor(elem1, elem2)
            )
        )

        pixel_neighbor_embeds = SequenceBatch.embed(pixel_neighbor_indices, base_dom_embeds_flat)
        pixel_neighbor_embeds_flat = SequenceBatch.reduce_mean(pixel_neighbor_embeds, allow_empty=True)

        # Combine all embeddings
        try:
            dom_embeds_flat = torch.cat([
                base_dom_embeds_flat,
                pixel_neighbor_embeds_flat,
                dom_neighbor_embeds
            ], 1)
        except RuntimeError as e:
            sizes = [
                base_dom_embeds_flat.size(),
                pixel_neighbor_embeds_flat.size(),
                dom_neighbor_embeds.size()
            ]
            raise RuntimeError(f"Failed to concatenate final embeddings: {e}\n"
                             f"Individual sizes: {sizes}")

        # Final reshape
        final_embed_dim = dom_embeds_flat.size(1)
        try:
            dom_embeds = dom_embeds_flat.view(batch_size, num_dom_elems, final_embed_dim)
            return dom_embeds
        except RuntimeError as e:
            raise RuntimeError(f"Failed final reshape: {e}\n"
                             f"Flat shape: {dom_embeds_flat.size()}\n"
                             f"Attempted shape: ({batch_size}, {num_dom_elems}, {final_embed_dim})")
