...
class TestMiniWoBEnvironment(MiniWoBTester):
    ...
    def test_run(self, env):
        ...
        assert all(list(x.fields.keys) == ['dummy'] for x in states)
...
class TestMiniWoBFields(MiniWoBTester):
    ...
    def test_fields(self, env):
        ...
        # Training time
        states = env.reset(seeds=[1, 2, 3])
        if states is None:
            print("Error: env.reset() returned None during training time")
            return
        for state in states:
        ...
        # Test time
        states = env.reset(mode='test',seeds=[1, 2, 3])
        if states is None:
            print("Error: env.reset() returned None during test time")
            return
        for state in states:
            if state is not None:
                print(state.utterance)
                print(state.fields)
                assert list(state.fields.keys) == ['dummy']
                assert state.utterance

        # Training time (again)
        states = env.reset(mode='train',seeds=[1, 2, 3])
        if states is None:
            print("Error: env.reset() returned None during second training time")
            return
        for state in states:
            if state is not None:
                print(state.utterance)
