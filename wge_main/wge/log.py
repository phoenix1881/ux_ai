...
class EpisodeLogger(object):
    ...
    #     with open(trace_path + '.json', 'w', encoding='utf8') as f:
    #         trace_dicts = [trace.to_json_dict() for trace in episode_traces]
    #         serializable_trace_dicts = convert_to_serializable(trace_dicts)
    #         json.dump(serializable_trace_dicts, f, indent=2)
    ...
    def _log_traces(self, episodes, label, train_step):
        ...
        def convert_to_serializable(obj):
            ...
            if isinstance(obj, dict):
                ...
                return convert_to_serializable(obj.to_json_dict())
            return obj
        ...
        with open(trace_path + '.json', 'w', encoding='utf8') as f:
            trace_dicts = [trace.to_json_dict() for trace in episode_traces]
            json.dump(trace_dicts, f, indent=2, cls=NumpyJSONEncoder)
        ...
        for i, ep in enumerate(episodes):
            ...
            for j, experience in enumerate(ep):
                ...
                path = join(img_path, '{}-{}-{}.png'.format(train_step, i, j))
                state.screenshot.save(path)
                actions.append(convert_to_serializable(action.to_dict()))
            # write action summary
            path = join(img_path, '{}-{}.json'.format(train_step, i))
            with open(path, 'w') as fout:
                json.dump(actions, fout, cls=NumpyJSONEncoder)
