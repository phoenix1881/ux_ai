...
class MiniWoBEnvironment(Environment):
    ...
    def reset(self, seeds=None, mode=None, record_screenshots=False):
        ...
        Returns:
            states (list[MiniWoBState]): List of states after resetting instances.
        """
        # If any instance has died, reconfigure them
        if any(instance.died for instance in self.instances):
            logging.warning('An instance died. Reconfiguring instances ...')
            self.configure(len(self.instances), seeds, **self.configure_kwargs)
        
        # Set seeds for instances
        if seeds is None:
        ...
        # Set screenshot recording option
        self.set_record_screenshots(record_screenshots)
        
        # Initialize states list for each instance
        states = [None] * len(self.instances)
        for i, instance in enumerate(self.instances):
            try:
                # Safely call reset for each instance
                instance.call(instance.reset, states, seeds[i])
            except Exception as e:
                logging.error(f"Error resetting instance {i}: {e}")
        
        # Wait for all instances to finish
        for instance in self.instances:
            instance.wait()
        
        # Mark environment as died if any instance died
