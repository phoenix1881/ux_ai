...
from wge.miniwob import instance as miniwob_instance
import prompt_util
...
BASE_URL = 'third_party/miniwob-sandbox/html/miniwob'

def create_logger(name, file_path, level='DEBUG'):
    """Create a logger with the specified name."""
    logger = logging.getLogger(name)
    
    # Convert level string to logging level using the logging constants
    level = getattr(logging, level.upper(), logging.DEBUG)  # Default to DEBUG if invalid level
...
logger = create_logger("Logger", "/Users/jeevana/Documents/GitHub/ux_ai_ui_explorer/log_file.log", "INFO")
...
class MiniWoBRunner:
  ...
  def model_inf(self, prompt, temperature=0.5, max_decode_steps=128):
    response = openai.chat.completions.create(
      model="gpt-4o",
      messages=[
        {"role": "system", "content": "You are a helpful assistant to  explore the UI. Give me directions to complete the given UI task. The directions should include step by step actions to take."},
    ...
    # print("Prompt = ", prompt)
    # print("\n\n")
    answer = [choice.message.content.strip() for choice in response.choices]
    # print("Answer = ",answer)
    # print("End of answer \n\n\n\n\n")
    processed_answer = [re.sub(r'```plaintext\n|\n```', '', ans) for ans in answer]
